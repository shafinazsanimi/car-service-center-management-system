<?php 
	...
	if(isset($_POST['query'])){
		...
		$stmnt=$con->prepare("SELECT * FROM sparepart WHERE sp_type='Tyre'");
	}
	$stmnt->execute();
	...
	if($result->num_rows>0){
		$output="<thead>
  				 ...
  				 while($row=$result->fetch_assoc()){
  				 	$output.="
					<tr class='clickable-row text-primary' data-href='/fyp/supervisor/sparepart/SparePartDisplay1.php?id=".$row['sp_id']."'>
  				 		<td><u>".$row['sp_id']."</u></td>
